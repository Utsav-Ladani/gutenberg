...
const { AsyncDependenciesBlock } = rspack;

const defaultExternalizedReportFileName = 'externalized-dependencies.json';

class DependencyExtractionRspackPlugin {
	constructor( options ) {
	...
	/**
	 * @param {rspack.ExternalItemFunctionData}                              data
	...
	apply( compiler ) {
		 ...
		 * @type {rspack.ExternalsPlugin}
		 */
		this.externalsPlugin = new rspack.ExternalsPlugin(
	 ...
	 * @param {rspack.Compilation} compilation
	 */
		...
		for ( const chunk of entrypointChunks ) {
			...
			const processModule = ( m ) => {
				...
				if ( this.externalizedDeps.has( userRequest ) ) {
					if ( this.useModules ) {
						const isStatic =
							DependencyExtractionRspackPlugin.hasStaticDependencyPathToRoot(
								compilation,
								m
							);
	 ...
	 *
	 * @param {rspack.Compilation}       compilation
	 * @param {rspack.DependenciesBlock} block
	 *
	 * @return {boolean} True if there is a static import path to the root
	 */
	static hasStaticDependencyPathToRoot( compilation, block ) {
		if ( DependencyExtractionRspackPlugin.#staticDepsCache.has( block ) ) {
			return DependencyExtractionRspackPlugin.#staticDepsCache.get(
				block
			);
		}

		if (
			DependencyExtractionRspackPlugin.#staticDepsCurrent.has( block )
		) {
			return false;
		}

		DependencyExtractionRspackPlugin.#staticDepsCurrent.add( block );
		...
		if ( ! incomingConnections.length ) {
			DependencyExtractionRspackPlugin.#staticDepsCache.set(
				block,
				true
			);
			DependencyExtractionRspackPlugin.#staticDepsCurrent.delete( block );
			return true;
		}
		...
		DependencyExtractionRspackPlugin.#staticDepsCache.set( block, result );
		DependencyExtractionRspackPlugin.#staticDepsCurrent.delete( block );
		return result;
	}
}

module.exports = DependencyExtractionRspackPlugin;
